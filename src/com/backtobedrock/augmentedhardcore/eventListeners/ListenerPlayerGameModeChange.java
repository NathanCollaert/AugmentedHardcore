...
public class ListenerPlayerGameModeChange extends AbstractEventListener {
    ...
    public void onPlayerGameModeChange(PlayerGameModeChangeEvent event) {
        ...
        if (ban != null && event.getNewGameMode() != GameMode.SPECTATOR) {
            player.sendMessage(String.format("§cCannot change game mode, you are still banned for another %s.", MessageUtils.getTimeFromTicks(MessageUtils.timeUnitToTicks(ChronoUnit.SECONDS.between(LocalDateTime.now(), ban.getValue1().getExpirationDate()), TimeUnit.SECONDS), TimePattern.LONG)));
